 ...
 * was found — Merlin cannot verify the link works, but can check for common patterns.
 *
 * Returns true if any of the following are present:
 * - Merlin template variables: {{unsubscribeUrl}} or {{manageUrl}}
...
export function detectUnsubscribeSignal(body: string): boolean {
  ...
  // Merlin's own managed unsubscribe variables
