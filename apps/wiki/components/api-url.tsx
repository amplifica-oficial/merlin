...
export function ApiUrl({path = ''}: {path?: string}) {
  const baseUrl = getRuntimeEnv('API_URI') || process.env.NEXT_PUBLIC_API_URI || 'https://next-api.merlin.example';
  return (
    <code>
      {baseUrl}
      {path}
...
export function DashboardUrl({path = ''}: {path?: string}) {
  const baseUrl =
    getRuntimeEnv('DASHBOARD_URI') || process.env.NEXT_PUBLIC_DASHBOARD_URI || 'https://next-app.merlin.example';
  return (
    <code>
      {baseUrl}
      {path}
