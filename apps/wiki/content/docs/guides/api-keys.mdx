---
title: API Keys
description: How Merlin's two-key model works and how to use, rotate, and revoke keys safely
icon: Key
---

Every project in Merlin has exactly two API keys: a **public** key and a **secret** key. Both authenticate requests to the same API but cover different surfaces.
...
```bash
curl https://next-api.merlin.example/v1/send \
...
```javascript
const response = await fetch('https://next-api.merlin.example/v1/send', {
  method: 'POST',
  headers: {
    'Authorization': `Bearer ${process.env.MERLIN_SECRET_KEY}`,
...
response = requests.post(
    'https://next-api.merlin.example/v1/send',
    headers={
        'Authorization': f"Bearer {os.environ['MERLIN_SECRET_KEY']}",
        'Content-Type': 'application/json',
    },
    json={
        'to': 'user@example.com',
...
- Use separate Merlin projects for staging and production so a leaked staging key can't touch production data.
