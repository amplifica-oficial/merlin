...
3. Optionally enable **Send confirmation email (double opt-in)** if you need imported contacts to confirm before receiving marketing email. New contacts are imported as unsubscribed and trigger a `contact.imported` event for each newly created contact.
...
When `doubleOptIn=true`, new contacts are created unsubscribed and each newly created contact emits a `contact.imported` event. Existing contacts are not downgraded. Wire a workflow on `contact.imported` with a transactional template containing `{{subscribeUrl}}` to send confirmation emails. See [Double opt-in](/recipes/double-opt-in).
...
- **Double opt-in imports**: enable **Send confirmation email (double opt-in)** in the import dialog (or pass `doubleOptIn=true` via the API). New contacts are imported as unsubscribed and emit `contact.imported`. Set up a workflow triggered on that event with a transactional template containing `{{subscribeUrl}}`. Existing contacts keep their current subscription status.
