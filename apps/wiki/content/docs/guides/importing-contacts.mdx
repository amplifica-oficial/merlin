...
- **`subscribed` column**: optional. When double opt-in is off, use `true`/`false`/`yes`/`no` to set subscription status on import. When double opt-in is on, this column is ignored for new contacts (they are always imported unsubscribed).
