...
If the key is new, Merlin processes the request normally. If your project has already used that key, Merlin **refuses** the request with `409` rather than performing it a second time.
...
```bash
curl -X POST https://api.merlin.example/v1/send \
...
```javascript
await fetch('https://api.merlin.example/v1/send', {
  method: 'POST',
  headers: {
...
requests.post(
    "https://api.merlin.example/v1/send",
    headers={
        "Authorization": "Bearer sk_your_secret_key",
        "Idempotency-Key": "receipt-order-1234",
    },
...
<Callout type="warn">
Merlin **refuses** a reused key; it does not replay the original response. A `409` tells you the operation was not performed twice, but it does not return the original email or event ID. If you need that ID, record it when the first request succeeds.
</Callout>
