---
title: Tracking
description: Track opens and clicks on the emails you send through Merlin
icon: HatGlasses
---

Merlin provides built-in tracking for email opens and link clicks so you can monitor engagement and drive workflows or segments off email activity.

## Open tracking

When open tracking is enabled, Merlin includes a small invisible tracking pixel at the bottom of your email. When the recipient's mail client loads the pixel, the open is recorded as an `email.open` event on the contact.
...
When click tracking is enabled, Merlin rewrites links in your email body to route through a tracking redirect. When a contact clicks a link, Merlin records the click and the original URL on an `email.click` event before forwarding the contact to the destination.
