...
Verifying your domain is a requirement to send emails through Merlin. Domain verification helps improve email deliverability and ensures that your emails are less likely to be marked as spam.

## Verifying a domain

You can verify a domain by adding it in the domain tab of the project settings. Once added, Merlin will provide you with the necessary DNS records to add to your domain's DNS settings.
...
You'll need to add **3 CNAME records** provided by Merlin. These records contain cryptographic keys that email receivers use to verify your emails are authentic.
...
A domain can only have **one** SPF TXT record. If you already use another email provider (Google Workspace, Microsoft 365, another sending platform), you must **merge** Merlin's SPF mechanism into your existing record — don't add a second SPF record. For example, if your existing record is `v=spf1 include:_spf.google.com ~all`, the merged version is `v=spf1 include:_spf.google.com include:&lt;Merlin's include from the dashboard&gt; ~all`. Two separate SPF records will cause both to fail.
</Callout>

### Bounce Handling (1 MX record)

This **MX record** allows Merlin to receive bounce notifications and spam complaints from email providers.
...
This MX record handles delivery feedback (bounces, complaints) for emails Merlin sends out — it's separate from the [inbound MX record](/guides/receiving-emails) used to **receive** emails sent to your domain. They serve different purposes; you can have either one or both.
</Callout>
...
Merlin doesn't require DMARC, but most major mailbox providers (Gmail, Yahoo, Microsoft) now expect it for bulk senders. Add a TXT record at `_dmarc.yourdomain.com`:
...
If you set this up, you'll need an additional MX record and TXT record on the subdomain — Merlin's dashboard will show you the exact values to add when you enable it on a verified domain.

## Verification Status

After adding all DNS records, Merlin automatically checks verification status in the background. Verification typically completes within a few minutes, but can take up to 72 hours depending on DNS propagation.
