...
Merlin automatically monitors the bounce and complaint rates of your emails to help maintain a health sender reputation. High bounce or complaint rates can negatively impact your deliverability and may lead to your account being suspended.

## Bounce Management

A bounce occurs when an email cannot be delivered to the recipient's inbox. Merlin records every bounce as an `email.bounce` event on the contact, and automatically unsubscribes the contact **only on permanent (hard) bounces**. Transient (soft) bounces don't change subscription state — they're typically retried by the upstream mail server.
...
A complaint occurs when a recipient marks your email as spam in their inbox provider. When Merlin receives that complaint, the contact is **always automatically unsubscribed** and Merlin fires an `email.complaint` event on the contact.
...
- Include a clear, working unsubscribe link in every marketing email — Merlin injects this automatically for `MARKETING` template/campaign types.
