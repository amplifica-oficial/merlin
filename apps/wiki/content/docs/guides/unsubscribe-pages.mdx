...
description: Merlin's hosted pages for letting recipients unsubscribe, resubscribe, and manage their email preferences
icon: UserX
---

Every email Merlin sends to a recipient who can unsubscribe (marketing or headless templates) carries a personalized link to a hosted page where they can manage their subscription. You don't need to build any of this — Merlin hosts the pages, handles the state changes, and tracks the events for you.
...
Where Merlin drops these links depends on the template type:

| Template type     | Auto footer with `{{unsubscribeUrl}}`?      | Notes                                                                  |
| ----------------- | -------------------------------------------- | ---------------------------------------------------------------------- |
| **Marketing**     | Yes — Merlin appends a localized footer      | You don't need to do anything. The footer respects the recipient's `locale`. |
