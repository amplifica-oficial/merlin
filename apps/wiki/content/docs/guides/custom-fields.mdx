...
Every contact in Merlin has a `data` object alongside the built-in fields (`email`, `subscribed`, `createdAt`, `updatedAt`). You decide what goes in `data` — first names, plan tiers, signup dates, internal user IDs, anything you want to use later in templates, segments, or workflow conditions.
...
Merlin infers a type for each field the first time it sees a non-null value in your project:
...
The inferred type powers the segment filter UI (showing date pickers for date fields, number inputs for numeric ones, etc.) and is sticky once set per project. If you mix types for a key, the original inference wins for the UI but Merlin still stores whatever value you send.
...
Some keys are managed by Merlin and **silently filtered out** of any `data` payload — sending them won't store them and won't return an error:

`id`, `merlin_id`, `merlin_email`, `email`, `unsubscribeUrl`, `subscribeUrl`, `manageUrl`
...
`locale` is a special soft-reserved key — it's stored under `data` like other fields, but Merlin reads it for [localization](/guides/localization).
...
The `?? fallback` syntax is Merlin-specific and lets you provide a default when the field is missing or null. Nested data (`data.profile.tier`) is accessible the same way: `{{profile.tier}}`.
