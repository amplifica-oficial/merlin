...
Contacts in Merlin represent an individual email recipient. Each contact has an identifier and is linked to an email address.

## Adding contacts

Contacts can be added to your Merlin project in several ways:
...
- Using [`POST /contacts`](/api-reference/contacts/createContact) — create or upsert a single contact. Existing emails are **updated**, not rejected; the response includes a `_meta: { isNew, isUpdate, confirmationSent }` block and uses status `201` for new contacts and `200` for updates. Pass `doubleOptIn: true` (and optionally `confirmationTemplateId`) to create the contact unsubscribed and send a confirmation email. See [Double opt-in](/recipes/double-opt-in).
...
- **Forms** — forms embedded in landing pages that upsert contacts, write tags for dynamic segments, and optionally add to static segments. See [Forms](/concepts/forms).
...
Contact data types are inferred from the **first non-null value** Merlin sees for a given key in your project:
...
Some keys are reserved by Merlin and managed at the contact level (not inside `data`). You can read them but you can't set them through `data`:
...
`id`, `merlin_id`, `merlin_email`, `unsubscribeUrl`, `subscribeUrl`, `manageUrl`
...
When `doubleOptIn` is true on `POST /contacts` or a CSV import, new contacts are created unsubscribed and receive a transactional confirmation email automatically. Existing contacts are not downgraded and do not receive another confirmation. See [Double opt-in](/recipes/double-opt-in).
...
- **Automatically** when a recipient marks one of your emails as spam (their mailbox provider reports the complaint back to Merlin).
