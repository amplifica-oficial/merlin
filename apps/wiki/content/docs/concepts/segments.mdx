...
Dynamic segments can opt into **Track membership changes**. When enabled, Merlin fires events whenever a contact enters or leaves the segment:
...
The member count updates immediately when you create or change a dynamic segment's filter. After that, Merlin recomputes membership in the background on a regular cadence — diffing current matches against the previous set, recording entries and exits, and emitting the corresponding events.
