...
Templates are stored in the Merlin dashboard and can be used in campaigns, workflows, and transactional emails sent through `/v1/send`.
...
New projects already include a transactional **Confirm your subscription** template with `{{subscribeUrl}}`, seeded for [double opt-in](/recipes/double-opt-in). Its `from` address starts as a placeholder (`noreply@yourdomain.com`). When you [verify the first domain](/guides/verifying-domains), Merlin replaces `yourdomain.com` in every template that still uses the placeholder (including `from`, `replyTo`, subject, and body).
...
Images uploaded from the editor are stored in the project's **Email images** folder. Manage them from [Files](/concepts/files).
...
The following fields are reserved by Merlin but can still be used.
...
There are three types of templates in Merlin. Each type is treated at the same priority when sending emails, you should not pick one type over the other based on deliverability or performance.

| Type          | Respects opt-out | Merlin unsubscribe footer | Description                                                                                                          |
| ------------- | :--------------: | :----------------------: | -------------------------------------------------------------------------------------------------------------------- |
| Marketing     | Yes              | Yes                      | Automatically includes a Merlin-hosted unsubscribe footer. Will not be sent to contacts who are unsubscribed          |
| Transactional | No               | No                       | Does not include any way to unsubscribe. Will be sent to any contact, regardless of subscription state               |
| Headless      | Yes              | No                       | Respects opt-out like marketing, but no Merlin footer is appended. You are responsible for providing an unsubscribe mechanism in the email body. Use `{{unsubscribeUrl}}` or `{{manageUrl}}` to link to Merlin's managed unsubscribe page |
