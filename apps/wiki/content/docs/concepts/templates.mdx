...
New projects already include a transactional **Confirm your subscription** template with `{{subscribeUrl}}`, seeded for [double opt-in](/recipes/double-opt-in). Its `from` address is a placeholder (`noreply@yourdomain.com`) — edit it after you [verify a domain](/guides/verifying-domains).
