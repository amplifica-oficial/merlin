...
| `ALLOWLIST_TRUSTED_DOMAINS`| No       | Controls who can create an account. `ALL_DOMAINS` = open signup. Comma-separated domains (e.g. `empresa.com`) = restricted to those domains plus emails on the allowlist. Empty = closed (allowlist only). | `ALL_DOMAINS` |

When allowlist mode is restricted or closed, configure `MERLIN_API_KEY` and `MERLIN_FROM_ADDRESS` so new users receive a verification email after signup. Users with a trusted-domain email can manage the allowlist from the **Authorization** page in the dashboard.
