...
| `DATABASE_URL`        | Yes      | Full PostgreSQL connection string used by the application at runtime (typically through PgBouncer). Auto-configured in Docker. | `postgresql://merlin:password@postgres:5432/merlin` |
| `DIRECT_DATABASE_URL` | Yes      | Direct PostgreSQL connection string used by Prisma migrations. Must bypass any connection pooler. Auto-configured in Docker.   | `postgresql://merlin:password@postgres:5432/merlin` |
...
| `SES_CONFIGURATION_SET`             | No       | SES configuration set name used for open/click tracking.                                                                                         | `merlin-configuration-set` (default)             |
| `SES_CONFIGURATION_SET_NO_TRACKING` | No       | A second SES configuration set without tracking. When set, projects can toggle email tracking on/off. If omitted, the tracking toggle is hidden. | `merlin-no-tracking-configuration-set` (default) |
| `MAIL_FROM_SUBDOMAIN`               | No       | Subdomain prefix used when constructing the MAIL FROM hostname for a verified domain (e.g. with default `merlin` and domain `yourdomain.com`, the MAIL FROM is `merlin.yourdomain.com`). Override when the default subdomain is already in use (e.g. by an R2/CDN custom domain), since the MAIL FROM hostname needs MX + TXT records that can't coexist with a CNAME. | `merlin`                                         |
...
When S3 is configured, the dashboard [Files](/concepts/files) library is enabled — not only template-editor image uploads.

Browser file uploads use a presigned `PUT` to the bucket. Configure CORS on the bucket in AWS (or Minio) — the app does not write bucket CORS. Allow the dashboard origin (`http://localhost:3000` in development and `DASHBOARD_URI` in production), methods `GET`, `PUT`, `HEAD`, and header `content-type`. If the same bucket is shared with another app, keep that app's origins too.

| Variable               | Required | Description                                                        | Default             |
| ---------------------- | -------- | ------------------------------------------------------------------ | ------------------- |
| `MINIO_ROOT_USER`      | No       | Minio root username (Docker Compose only).                         | `merlin`             |
| `MINIO_ROOT_PASSWORD`  | No       | Minio root password (Docker Compose only).                         | `merlinminiopass`    |
...
| `S3_REGION`            | No       | AWS/Minio region. Must match the bucket region (AWS HeadBucket returns 400 if this is wrong). | `us-east-1` |
...
| `S3_PUBLIC_URL`        | No       | Publicly accessible base URL for stored files. `S3_BEAUTIFUL_DOMAIN` is accepted as an alias. | — |
| `S3_FORCE_PATH_STYLE`  | No       | Use path-style URLs instead of virtual-hosted. Required for Minio. Set `false` for AWS S3. | `true`              |

## Attachments

Merlin supports attachments on transactional emails. AWS SES caps total message size at 40 MB; the defaults below leave headroom but can be tuned for your use case.
...
When configured, Merlin will send email notifications to users for critical events (e.g. project disabled, billing limits reached). Without these, only ntfy notifications are sent.

| Variable             | Required | Description                                                | Example                  |
| -------------------- | -------- | ---------------------------------------------------------- | ------------------------ |
| `MERLIN_API_KEY`      | No       | API key for a Merlin instance to send transactional emails. | `pk_...`                 |
| `MERLIN_FROM_ADDRESS` | No       | From address used for platform notification emails.        | `noreply@yourdomain.com` |

## Notifications (ntfy)

Merlin bundles a self-hosted [ntfy](https://ntfy.sh) server for internal system notifications.

| Variable    | Required | Description                                                                         | Default                           |
| ----------- | -------- | ----------------------------------------------------------------------------------- | --------------------------------- |
| `NTFY_PORT` | No       | Port for the ntfy web UI (Docker Compose only).                                     | `8080`                            |
| `NTFY_URL`  | No       | ntfy topic URL. Change this to use an external ntfy.sh server or your own instance. | `http://ntfy/merlin-notifications` |

## User Management

| Variable                   | Required | Description                                                                                                                                                                                                 | Default       |
| -------------------------- | -------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------- |
| `DISABLE_SIGNUPS`          | No       | When `true`, the signup endpoint rejects new registrations. Useful for private instances. Takes precedence over allowlist settings.                                                                         | `false`       |
| `DISABLE_PASSWORD_AUTH`    | No       | When `true`, disables email+password login, signup, and password reset. OAuth (Google/GitHub) remains available if configured. Configure at least one OAuth provider, otherwise no login method will be available. | `false`       |
...
| `ALLOWLIST_TRUSTED_DOMAINS`| No       | Controls who can create an account and who can create new projects. `ALL_DOMAINS` = open signup and anyone can create projects. Comma-separated domains (e.g. `empresa.com`) = restricted to those domains plus emails on the allowlist; only trusted-domain users can create projects. Empty = closed (allowlist only). Allowlisted external users can sign in but must be shared projects from the **Authorization** page before they see any project. | `ALL_DOMAINS` |

When allowlist mode is restricted or closed, configure `MERLIN_API_KEY` and `MERLIN_FROM_ADDRESS` so new users receive a verification email after signup. Users with a trusted-domain email can manage the allowlist from the **Authorization** page in the dashboard and optionally share their admin/owner projects with allowlisted emails before those users sign up.
...
| `ALLOW_SHARED_DOMAINS`        | No       | When `true`, allows the same domain to be linked to multiple projects, bypassing the cross-project ownership check. Useful for self-hosted environments where multiple projects share the same sending domain. | `false` |
| `EMAIL_RATE_LIMIT_PER_SECOND` | No       | Override the email sending rate limit. If not set, Merlin automatically fetches the quota from your AWS SES account.                                        | —       |
...
Merlin can use AI to detect and block phishing emails before they're sent. Requires an [OpenRouter](https://openrouter.ai) API key.
