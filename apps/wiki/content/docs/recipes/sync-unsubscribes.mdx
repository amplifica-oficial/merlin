...
description: Mirror Merlin's subscription state into your own user table using a workflow + webhook
icon: RefreshCw
---
...
```ts
app.post('/merlin/unsubscribes', async (req, res) => {
  if (req.header('authorization') !== `Bearer ${process.env.MERLIN_WEBHOOK_SECRET}`) {
...
- Add a `WEBHOOK` step:
  - **URL**: `https://api.example.com/merlin/unsubscribes`
...
```bash
curl -X PATCH https://next-api.merlin.example/contacts/cnt_abc \
