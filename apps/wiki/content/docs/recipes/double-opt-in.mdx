...
<Callout title="No-code alternative" type="info">
  ...
  For CSV imports, enable **Send confirmation email (double opt-in)** in the import dialog. New contacts are imported unsubscribed and emit `contact.imported` — wire a workflow on that event the same way. The CSV `subscribed` column is ignored while double opt-in is enabled.
</Callout>
...
2. New contacts are created unsubscribed. Each newly created contact emits `contact.imported`.
...
Existing contacts in the CSV are updated in place but keep their current subscription status — they do not receive a new confirmation email. The CSV `subscribed` column is ignored while double opt-in is enabled.
