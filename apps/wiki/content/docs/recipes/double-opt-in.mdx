...
<Callout title="No-code alternative" type="info">
  ...
  For CSV imports and **Add Contact**, enable **Send confirmation email (double opt-in)** and pick a transactional template. New contacts are created unsubscribed and Merlin sends the confirmation email immediately. The last template you used is remembered in the browser. The CSV `subscribed` column is ignored while double opt-in is enabled.
</Callout>
