...
The Merlin API uses standardized error responses to help you quickly identify and resolve issues. All errors include:
...
```typescript
try {
  const response = await fetch('https://next-api.merlin.example/v1/track', {
    method: 'POST',
    headers: {
...
response = requests.post(
    'https://next-api.merlin.example/v1/track',
    headers={
...
- Check [status.merlin.example](https://status.merlin.example) for incidents.
