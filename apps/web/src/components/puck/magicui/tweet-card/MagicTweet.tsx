...
export const TweetMedia = ({tweet}: {tweet: EnrichedTweet}) => {
  if (!tweet.video && !tweet.photos) {
    ...
    const cardThumbnail = tweet?.card?.binding_values?.thumbnail_image_large?.image_value?.url as
      | string
      | undefined;
  ...
  return (
    <div className="flex flex-1 items-center justify-center">
      {tweet.video && tweet.video.variants[0] && (
        <video
...
export interface MagicTweetProps extends TweetCardDisplayOptions {
  tweet: Tweet;
  className?: string;
}
...
export const MagicTweet = ({
  ...
  return (
    <div
      ...
      {...props}
    >
      {showHeader ? <TweetHeader tweet={enrichedTweet} /> : null}
      {showBody ? <TweetBody tweet={enrichedTweet} /> : null}
      {showMedia ? <TweetMedia tweet={enrichedTweet} /> : null}
    </div>
  );
};
